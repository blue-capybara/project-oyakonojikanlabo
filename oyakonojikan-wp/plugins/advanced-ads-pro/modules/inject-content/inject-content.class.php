...
class Advanced_Ads_Pro_Module_Inject_Content {
	...
	private function add_skip_paragraph_filters() {
		...
		foreach ( $placements as $placement_id => $placement ) {
			if ( isset( $placement['type'] )
				&& 'post_bottom' === $placement['type']
				&& ! empty( $placement['options']['words_between_repeats'] ) ) {
				add_filter( 'advanced-ads-can-inject-into-content-' . $placement_id, array( $this, 'maybe_skip_after_content_placement' ), 10, 3 );
			}
		}
	}

	/**
	 * Check if the "After Content" placement has enough words before.
	 *
	 ...
	 * @return bool
	 */
	public function maybe_skip_after_content_placement( $return, $content = '', $placement_id ) {
		if ( ! $return ) {
			return false;
		}
