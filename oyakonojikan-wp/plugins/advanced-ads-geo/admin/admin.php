...
class Advanced_Ads_Geo_Admin {
	...
	public function render_settings_database() {
		...
		if ( $correct_databases ) {
			?><p><?php _e( "Geo Database found.", 'advanced-ads-geo' ); ?></p><?php
		}
                ...
                if( $ip ){
                    try {
                        ...
                        if( $reader ){
                            ...
                            $error = __( "Geo Database not found.", 'advanced-ads-geo' ) . '&nbsp;' . sprintf( __( 'Please read the %1$sinstallation instructions%2$s.', 'advanced-ads-geo' ), '<a href="' . ADVADS_URL . 'manual/geo-targeting-condition/#Enabling_Geo-Targeting" target="_blank">', '</a>' );
                        }
