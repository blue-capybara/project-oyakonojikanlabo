...
class WA_Content_Twitter extends WA_Content_Abstract {
	public $name = 'Twitter';
	...
	public function prepare( array &$contents, array $file_array ) {
		...
		} else if ( ! preg_match( '#https?://(www\.)?twitter\.com/.+?/status(es)?/.*#i', $contents['data'] ) ) {
			$contents['error'] = '正しいツイートのURLを入力してください。';
		}
	}
	...
	public function get_template() {
		ob_start();
		?>
		<script type="application/x-template" id="tmpl-twitter">
			<div class="editable-content__item">
				...
				<div class="editable-content__body">
					<div class="editable-content__title">
						<div class="editable-content__title-text">Twitter</div>
					...
					<dl class="field-list">
						<dd class="field-list__content"><input name="<?php echo WA_Content::get_field_key( true ) ?>[<%= i %>][data]" type="text" value="<%= value || '' %>" placeholder="ツイートへのリンクを貼り付けてください。">
							<% if (error) { %>
					...
					<input type="hidden" name="<?php echo WA_Content::get_field_key( true ) ?>[<%= i %>][type]" value="twitter">
				</div>
			</div>
		</script>
		<?php
