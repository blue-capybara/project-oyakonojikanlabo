...
class WA_Content_Image_With_Text extends WA_Content_Abstract {
	public $name = '画像＋テキスト';
	...
	public function prepare( array &$contents, array $file_array ) {
		...
		if ( ! IWF_Validation::not_empty( $contents['data']['desc'] ) ) {
			$contents['error']['desc'] = '必須入力です。';
		}
	}
	...
	public function get_html( array $contents ) {
		ob_start();
		?>
		<div class="wp-caption align-left">
		...
		<p><?php echo nl2br( $contents['data']['desc'] ) ?></p>
		<?php
		return ob_get_clean();
	}
	...
	public function get_template() {
		ob_start();
		?>
		<script type="application/x-template" id="tmpl-image-with-text">
			<div class="editable-content__item">
				...
				<div class="editable-content__body">
					<div class="editable-content__title">
						<div class="editable-content__title-text">画像＋テキスト</div>
					...
					<dl class="field-list">
						...
						<% if (value.image_file) { %>
						<dd class="field-list__content"><img src="<%=raw value.image_url %>" alt="" width="200"></dd>
						...
						<dt class="field-list__title">画像引用元名称</dt>
						...
						<dt class="field-list__title">画像引用元URL</dt>
						...
						<dt class="field-list__title">テキスト</dt>
						<dd class="field-list__content"><textarea name="<?php echo WA_Content::get_field_key( true ) ?>[<%= i %>][data][desc]" data-name="desc" rows="6"><%= value.desc || '' %></textarea>
							<% if (error.desc) { %>
							<p class="field-list__help -error"><%= error.desc %></p>
							<% } %>
						</dd>
					</dl>
					<input type="hidden" name="<?php echo WA_Content::get_field_key( true ) ?>[<%= i %>][order]" class="order-number" value="<%= i %>">
					<input type="hidden" name="<?php echo WA_Content::get_field_key( true ) ?>[<%= i %>][type]" value="image-with-text">
				</div>
			</div>
		</script>
		<?php
