...
<?php		
		...
		foreach ($posts_series as $post_series) {
			...
			if ($post->post_status == 'publish') {
				...
				if ($post_series->post_id == $post_id) {
					...
					$link = $this->get_frontend_permalink( $post_series->post_id );
				}
	...
	private function get_frontend_permalink( $post_id ) {
		$permalink = get_permalink( $post_id );
		if ( empty( $permalink ) ) {
			return '';
		}

		$frontend_base = apply_filters( 'easy_series_frontend_base_url', '' );
		if ( ! empty( $frontend_base ) ) {
