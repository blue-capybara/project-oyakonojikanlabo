...
class Advanced_Ads_Responsive_Amp {
	...
	public function wp() {
		...
		if ( function_exists( 'is_amp_endpoint' ) ) {
			...
			add_action( 'wp_footer', array( 'Advanced_Ads_Responsive_Amp', 'add_adsense_auto_ads' ) );
	...
	public function exclude_placements( $placements ) {
		...
		foreach( $placements as $_k => $_placement ){
			...
			if ( ! in_array( $_placement['type'], array( 'default', 'post_top', 'post_bottom', 'post_content' ) )
				...
				switch ( $content->unitType ) {
					...
					case 'responsive':
						if ( 'enable' == $fw ) {
							...
							return $output_part . sprintf( 'width="100vw" height="%dvw" data-full-width><div overflow></div></amp-ad>', absint( 100 * $height / $width ) );
						} else {
